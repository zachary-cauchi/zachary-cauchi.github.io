...
const categorisedSkills = computed(() => {
    return categories.value
        .map((category) => ({
            id: category.id,
            title: category.title[locale.value],
            skills: skills.value.filter(
    ...
    <article class="resume active" data-page="resume">
            ...
            <ol class="timeline-list">
                ...
                >
                    ...
                    <span class="timeline-date">
                        <span class="timeline-datefrom">
                            {{
                                new Date(
                                    work.dateFrom.year,
                                    work.dateFrom.month - 1,
                                    work.dateFrom.day,
                                ).toLocaleDateString(locale)
                            }}
                        </span>
                        —
                        <span class="timeline-dateto">
                            <span
                                v-if="
                                    'ongoing' in work.dateTo &&
                                    work.dateTo.ongoing == true
                                "
                                >Ongoing</span
                            >
                            <span v-else>
                    ...
                    <h5 class="h4 timeline-item-title">
                        <span>
                            <a :href="work.website[locale]" target="_blank"
                                >{{ work.employer[locale] }} 🌐</a
                            >
                        </span>
                        <span
                            v-show="work.department[locale]"
        ...
        <section class="skill">
            <div class="skills-container">
                ...
                <div
                    v-for="categorisedSkill in categorisedSkills"
                    :key="categorisedSkill.id"
                    class="skills-container content-card"
                >
                    <h4 class="h4">{{ categorisedSkill.title }}</h4>
                    <ul class="skills-list">
                        <li
                            v-for="skill in categorisedSkill.skills"
                            :key="skill.id"
                            class="skills-item"
                        >
                            <div class="title-wrapper">
                                <Icon
                                    :icon="skill.icon"
                                    class="text-ranko-500 text-[3rem]"
                                    style="width: 2rem"
                                />
                                <h5 class="h5">
                                    {{ skill.name }}
